...
---Install a launchd service (copy .plist to correct place and load)
...
function launchctl.get_service_status(label, options)
    options = options or {}
    local exit_code, stdout, stderr = launchctl.exec({ "print", "system/" .. label }, options)
    if exit_code == 0 then
        return "loaded", stdout
    else
        return "unloaded", stderr
    end
end
