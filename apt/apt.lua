...
local function _execute(cmd, args, options)
    ...
    rd:set_nonblocking(true); rderr:set_nonblocking(true)
...
local function install(dependencies, options)
    ...
    for i, dependency in ipairs(dependencies) do
        ...
        if not success then
            _debug{msg = "Failed to install dependency - " .. dependency .. ". APT stopped...", dependency = dependency, dependencies = dependencies}
